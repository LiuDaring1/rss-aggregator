# -*- coding: utf-8 -*-
"""build5.py — 组装 rev.5（v0.6 定向修订：评论重做+示范独立成块）"""
import json, os, sys
...
def gen_tree(item):
    ...
    for bi, br in enumerate(branches):
        for ch in br["children"]:
            rows.append((bi, ch["blank"], bool(ch.get("tall"))))
...
def build(pages=None):
    pages = pages or {}
    H = ['<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><title>口语素材周刊 · 2026年9月第1期（试刊）</title><link rel="stylesheet" href="style5.css"></head><body>']
...
<section class="fu">{modn}
  ...
  <div class="cu-blk"><div class="lbl">积累</div><p class="ctx"><b>值得记住的表达</b>　{f['memorize']}</p></div>
  <div class="cu-blk"><div class="lbl">借鉴</div><p class="ctx"><b>可以借鉴的讲法</b>　{f['method']}</p></div>
  <div class="demoblock">
    <div class="demo-cap">示范｜换个话题，可以这样说</div>
    <p class="demo-text">{f['demo']}</p>
  </div>
</section>""")
